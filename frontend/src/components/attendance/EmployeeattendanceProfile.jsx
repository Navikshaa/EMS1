...
const EmployeeProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchAttendance = async () => {
      try {
        ...
        const onTimeCount = filtered.filter(e => e.status?.includes("on time")).length;
  ...
  const averageTime = (times) => {
    ...
    const total = times.reduce((sum, d) => sum + (d.getHours() * 60 + d.getMinutes()), 0);
        ...
        <div className="flex flex-wrap md:flex-nowrap items-center mt-6">
          ...
          <div className="ml-6">
            ...
            <div className="flex flex-wrap gap-16 text-sm text-gray-300 mt-4">
              <p className="uppercase">ROLE <span className="font-semibold ml-2">{employee.role}</span></p>
              <p>PHONE NUMBER <span className="ml-2 font-semibold">+91 - {employee.phone}</span></p>
              <p>EMAIL <span className="ml-2 font-semibold">{employee.userId.email}</span></p>
            </div>
          </div>
        </div>
